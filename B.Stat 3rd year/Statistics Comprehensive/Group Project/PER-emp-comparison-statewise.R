...
for(num in setdiff(4:36, 31)){

   state <- state.codes.char[num] %>% strsplit(split = "") %>% unlist()
   cat("\n\n",state.codes.char[num],state.codes$State[num],"\n\n")
   
   temp.urban.v1.state <- temp.urban.v1 %>% filter(V13 == state[1] & V14 == state[2]
                                                   & V41 == "3")
   temp.urban.rv.state <- temp.urban.rv %>% filter(V13 == state[1] & V14 == state[2]
                                                   & V41 == "3")
